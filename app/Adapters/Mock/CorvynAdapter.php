...
class CorvynAdapter extends BaseServiceAdapter
{
    ...
    protected function generateHeaders(string $rawBody): array
    {
        ...
        // Use the timestamp from the incoming request header if present; otherwise generate one.
        $incomingHeaders = $this->currentRequest?->headers ?? [];
        $headerTimestamp = trim((string) ($incomingHeaders[self::HEADER_TIMESTAMP] ?? ''));
        $timestamp = ($headerTimestamp !== '') ? (int) $headerTimestamp : Carbon::now()->timestamp;
    ...
    protected function generateSignature(int $timestamp, string $rawBody, string $secret): string
    {
        ...
        $secret = trim($secret);
        $rawBody = trim($rawBody === '' ? 'undefined' : $rawBody);
